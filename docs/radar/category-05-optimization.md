...
> **成熟度**: 🟡 成长期 · **综合评分**: **0.58**
>
> **业界判断**：七个进化框架 + SkillReducer Token 优化，**技术路径最丰富**，但各自为政缺标准。**没有任何进化框架在循环中加入安全扫描**。
...
| 产品 | 来源 | 进化信号 | 关键数字 |
|---|---|---|---|
| [OpenSpace](tech-openspace) | HKU (4.7K+ ★) | FIX / DERIVED / CAPTURED 三模式 | 220 任务 40.8% → 70.8%, Token -46% |
| [Hermes Self-Evolution](tech-hermes-self-evolution) | Nous Research | GEPA 优化器 + PR 审核 | ICLR 2026 Oral |
| [SkillClaw](tech-skillclaw) | AMAP-ML | 多用户轨迹聚合 | Qwen3-Max 验证 |
| [MetaClaw](tech-metaclaw) | OpenClaw (3.5K+ ★) | Fast Loop + Slow Loop 双环 | Kimi-K2.5 21.4% → 40.6% |
| [SkillReducer](tech-skillreducer) | 港科大 | 静态压缩 + 渐进披露 | **55K Skill, 压缩后 +2.8%** |
| [EvoSkill (Pareto)](tech-evoskill) | Sentient AGI | Pareto 前沿 + Git 分支 | 零样本迁移 +5.3% |
| [SkillForge (Optimizer)](tech-skillforge) | 阿里云 | 三阶段失败归因 | 阿里云 5 场景 |
...
SkillReducer 大规模实证：**26.4% Skill 没写 description、60%+ body 是非可操作内容**。压缩后质量反升 2.8% —— 干扰 Agent 注意力的噪音少了。但仅静态压缩，**未做动态按需加载**。

### 🚨 进化 + 安全的交叉空白

> 7 个进化框架都在自动生成新 Skill，**没有一个在进化循环中检查"进化出来的 Skill 是否安全"**。进化可能无意中引入漏洞 —— 比如"更高效"的 Skill 通过跳过权限检查实现高效。
