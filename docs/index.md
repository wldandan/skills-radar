...
hero:
  ...
  tagline: 基于 skill-insight 全生命周期管理方法论 · 24 个公开可验证的产品 / 6 大生命周期
  image:
    src: /logo.svg
    alt: Skills Radar
  actions:
...
features:
  ...
  - icon: 🚀
    ...
    details: OpenSpace · ACE · Hermes Self-Evolution · SkillClaw · MetaClaw · SkillReducer · EvoSkill · SkillForge · SkillRL
  - icon: 🛡️
    title: Skill 管理 · 0.40 🟡
...
::: tip 🟢 已成熟：召回 / 优化
SkillRouter 在 80K 池上达 **74% Hit@1** 并工程化（1.2B 模型比 16B 基线少 13× 参数、快 5.8×）；SkillRet 17,810 Skill 检索 benchmark 上线。**优化赛道**有 ACE（ICLR 2026 接收的 Stanford context engineering）+ 8 个进化框架，技术路径最丰富。
:::
...
| [ACE](/radar/tech-ace) ★ | Stanford 等 (ICLR 2026) | 优化 / 生成 | +10.6% 智能体 / +8.6% 金融 |
| [Vercel skills.sh](/radar/tech-skills-sh) ★ | Vercel | 管理 / 召回 | **90,000+ Skill / 19 Agent 框架** |

★ = 本次更新新增（基于 2026 Q1-Q2 调研）
...
```
   ...
   Skill Seekers     AgentSkillOS    AgentSkillOS   SkillNet 5维   ACE            SkillNet
   EvoSkill          SkillNet        Memento-Skills EvoSkills       Hermes         AgentSkillOS
   EvoSkills         SkillRouter.org SkCC ★         SkillForge      SkillClaw      Agent Skills 标准
   Claudeception                                    SkillRet ★      MetaClaw
   Trace2Skill                                                      SkillReducer
   SkillForge                                                       EvoSkill
   SkillX                                                           SkillForge
   SkillRL ★                                                        SkillRL ★

成熟度:   🟡 0.55        🟢 0.65        🟡 0.45        🟡 0.47        🟢 0.60        🟡 0.40
```

每个阶段的产品都在推动 Agent 能力边界扩展，共同构成完整的 Skill 生态系统。**召回 / 优化已进入成熟期；生成 / 执行 / 评测仍在成长期；管理因安全防御工具空白而面临攻防不对称。**
